...
**The app is offline-first: every read is served from memory, every write lands locally first, and PostgreSQL is a sync backend.** The whole working set (profiles, accounts, categories, transactions — a few MB) is replicated into IndexedDB and held in a Zustand store; there are no route loaders and no data-fetching layer. `src/api/sync.functions.ts` holds the only two data endpoints, `pullChanges` and `pushChanges` — everything else under `src/api/` is auth, or a cookie/housekeeping call that is not a data endpoint at all. See `docs/offline-first-sync.md` for the plan and which phases have landed.
