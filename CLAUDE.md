...
- **Read path: `pullChanges` → IndexedDB → Zustand store → pure derivations.** A keyset-paginated delta pull, scoped to the caller's user, that sends tombstones as rows with `deletedAt` set. `src/modules/sync/` holds the IndexedDB wrapper (`idb.ts`), the store and its boot/pull/push orchestration (`useSyncStore.ts`), the loading gate (`SyncGate.tsx`) and the shared row/cursor/mutation types (`sync-types.ts`).
