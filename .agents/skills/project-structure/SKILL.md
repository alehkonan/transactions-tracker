...
2. **`src/api/*.ts` filenames are kebab-case**, not camelCase (`currency-rates.server.ts`, `selected-profile.server.ts`, `transactions-import.functions.ts` — not `currencyRates.server.ts`). This is the opposite of the `src/components/` rule below (PascalCase) — components are one-per-file classes/functions named after the thing they render, `src/api/` files are domain modules named after the domain.
3. **`src/database/getDb.server.ts` must have no top-level side effects.** The DB connection is created lazily inside `getDb()` so the TanStack Start compiler can tree-shake the postgres driver out of the client bundle. Never instantiate the client at module scope.
