...
function findLines(bin) {
  ...
  if (real.length >= 3) {
    const hs = real.map((l) => l.y1 - l.y0).toSorted((a, b) => a - b);
...
export function isolateDominant(bin, lines, { tol = 0.28 } = {}) {
  ...
  const clusters = [];
  for (const x of [...ms].toSorted((a, b) => b.m.cap - a.m.cap)) {
...
export function _debugLines(img) {
  ...
  return lines.map((ln) => {
    ...
    const hs = m.hs.map((h) => +(h / m.R).toFixed(2)).toSorted((a, b) => a - b);
    const hist = {};
    for (const h of hs) {
