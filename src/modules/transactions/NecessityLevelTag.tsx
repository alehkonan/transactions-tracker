...
export function NecessityLevelTag({ level }: Props) {
  return (
    <span
      className={twMerge(
        "inline-block w-24 rounded-full border px-2 py-0.5 text-center text-xs font-medium whitespace-nowrap capitalize",
        necessityLevelStyles[level],
      )}
    >
      {level.toLowerCase()}
    </span>
