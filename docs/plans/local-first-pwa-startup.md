...
Статус: **реализация интегрирована; выпуск заблокирован непройденными compatibility gates**. Дата плана: 2026-09-11. Фактические результаты: [release checklist](../release-checklists/local-first-pwa-startup.md).
...
> **Статус исполнения (2026-09-12): реализация завершена, browser gates требуют запуска перед выпуском.** Готовы единый coordinator для password/passkey/autofill, общий browser lock, lifecycle transitions, owner-preserving same-user completion, durable `syncAuth: unknown | authenticated | login-required | owner-mismatch`, fail-closed sync admission после failed/crashed auth, межвкладочные `auth-completed`/`auth-required`, безопасный return path и fencing CSV import callbacks. Все открытые form actions привязаны к исходному `replicaId`, workspace remounts на replacement, а delayed push/status callbacks повторно проверяют captured context. Sign-out теперь в одной IDB transaction одновременно считывает актуальную outbox obligation и ставит lifecycle marker; изменение queue после открытия confirmation требует повторного подтверждения. Login явно предлагает discard/sign-out для bound replica другого пользователя. Auth E2E workflow переведён на стабильные `data-testid`; `playwright.config.ts` намеренно оставлен минимальным с одним Chromium и без отдельного PWA-проекта. Добавлены targeted regression cases в `complete-sign-in.test.ts` и `mutations.test.ts`; peer-tab/crash recovery browser gates из строки 329 необходимо выполнить перед выпуском.
...
- [x] ADR 0003 реализован: expiry не запрещает local просмотр/изменения.
- [x] Worker на свежем production build действительно регистрируется и кэширует полную анонимную оболочку.
- [x] Warm online/offline запуск не ждёт сервер; initial bootstrap честно отличается от local boot.
- [x] Same-user reauth сохраняет exact obligations; expected-owner protocol защищает все sync входы.
- [x] Миграция и delayed callbacks не теряют и не смешивают данные.
- [x] Выбор/создание профиля локальны; stale-replica recovery не стирает рабочий набор до успешной замены.
- [x] Обновления не перезагружают dirty формы; failed install сохраняет рабочую версию.
- [x] Background delivery либо безопасно реализован по ADR 0001, либо явно отключён и задокументирован.
- [x] Матрица приёмки имеет реальные локальные результаты; непройденные browser/deploy gates перечислены, не объявлены успешными.
- [ ] Cutover/rollback проверены или выпуск заблокирован до выполнения явно указанных шагов.
- [x] Документация соответствует реализованному состоянию; тестовые credentials и финансовые артефакты не попали в repo.
